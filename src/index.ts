...
<p class="hint">curl でも使えます: <code>curl [hostname]/${label.replace('github.com/', '')}</code></p>
</div></body></html>`
}
...
<h2>使い方</h2>
<pre>curl [hostname]/&lt;username&gt;
curl [hostname]/&lt;username&gt;.json
curl [hostname]/&lt;username&gt;/badge
curl "[hostname]/?keys=ssh-ed25519 AAAA..."
curl [hostname]/user1,user2,user3</pre>
...
${'═'.repeat(42)}
使い方:
  curl <hostname>/<username>
  curl <hostname>/<username>.json
  curl <hostname>/<username>/badge
  curl "<hostname>/?keys=<pubkey>"
  curl <hostname>/user1,user2,user3
`)
})
